/**
 * @brief Function to convert string of the format DD/MM/YYYY to my_date_t structure
 * 
 * @param input_string String which needs to be converted
 * @param result_time Resultant structure to hold the converted string
 * @return status_t SUCCESS if conversion is done, status_t code otherwise.
 */




#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>

typedef enum status_t {
    SUCCESS,     // Function has successfully converted the string to structure
    NULL_PTR,    // Function is given NULL pointers to work with
    INCORRECT    // Incorrect values for date or month or year
} status_t;

typedef struct my_date_t {
    uint8_t date;
    uint8_t month;
    uint16_t year;
} my_date_t;

status_t string_to_date_converter(char* input_string, my_date_t* result_date) {
    if (input_string == NULL || result_date == NULL) {
        return NULL_PTR;
    }
    
    int day, month, year;
    if (sscanf(input_string, "%d/%d/%d", &day, &month, &year) == 3) { //On success, the function returns the number of variables filled. In the case of an input failure before any data could be successfully read, EOF is returned.
        if (day < 1 || day > 31 || month < 1 || month > 12||year>9999 ||year<0) {//range of day,date and year are specified so that if any value is out of range it will return incorrect
            return INCORRECT;
        }
        
        result_date->date = (uint8_t)day;
        result_date->month = (uint8_t)month;
        result_date->year = (uint16_t)year;
        
        return SUCCESS;
    } else {
        return INCORRECT;
    }
}

int main() {
    char date_string[11];  // Array to store the DD/MM/YYYY formated input date string
    printf("Enter the date in DD/MM/YYYY format : ");
    scanf("%s", date_string);
    
    my_date_t result;
    status_t conversion_status = string_to_date_converter(date_string, &result);
    
    if (conversion_status == SUCCESS) {
        printf("Date: %d\n", result.date);
        printf("Month: %d\n", result.month);
        printf("Year: %d\n", result.year);
    } else if (conversion_status == NULL_PTR) {
        printf("NULL pointers provided, Try again!\n");
    } else if (conversion_status == INCORRECT) {
        printf("Incorrect date format, Try again!\n");
    }
    
    return 0;
}